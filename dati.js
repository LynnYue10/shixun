...
$('#dati').on("keydown", function (event) {
...
function TableInit() {
    ...
    oTableInit.Init = function () {
        $('#userTable').bootstrapTable({
            url: httpRequestUrl + '/answer/queryUserList',         //请求后台的URL（*）
            method: 'POST',                      //请求方式（*）
                ...
                if(res.code == "666"){
                    ...
                    if (userInfo.length) {
                        for (var i = 0; i < userInfo.length; i++) {
                            ...
                            dataNewObj.groupName = userInfo[i].groupName;
...
function addFunctionAlty(value, row, index) {
    ...
    btnText += "<button type=\"button\" id=\"btn_look\" onclick=\"resetPassword(" + "'" + row.id + "'" + ")\" style='width: 77px;' class=\"btn btn-default-g ajax-link\">答题</button>&nbsp;&nbsp;";

    btnText += "<button type=\"button\" id=\"btn_stop" + row.id + "\" onclick=\"deleteUser(" + "'" + row.id + "'" + ")\" style='width: 77px;' class=\"btn btn-danger-g ajax-link\">查看结果</button>&nbsp;&nbsp;";
    return btnText;
}
